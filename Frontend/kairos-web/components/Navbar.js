...
export default function Navbar() {
    ...
    // --- MODIFIED: Your existing link logic is now aware of the user's state ---
    const getDisplayLinks = () => {
        ...
        // If a user is logged in, never show Login or Signup
        if (user) {
            switch (pathname) {
        ...
        // If no user is logged in, show the original links
        switch (pathname) {
    ...
    // --- Your existing styling functions (no changes needed) ---
    ...
    const getMobileLinkStyles = (link) => {
        return link.href === pathname ? "text-white bg-white/10" : "text-[#a7ebf2] hover:bg-white/10";
    };
    ...
    return (
        <nav className="bg-[#011c40] p-4 shadow-md relative z-40">
            <div className="mx-auto flex justify-between items-center">
                {/* Logo */}
                <Link href="/" className="z-20 flex items-center gap-3 text-[#a7ebf2] text-2xl font-bold tracking-wider whitespace-nowrap" onClick={() => setIsOpen(false)}>
                    <Image src={"/logo-option-1.svg"} width={40} height={40} alt="KAIROS logo" />
                    <div>KAIROS</div>
                </Link>
                ...
                <div className="flex items-center gap-4">
                    ...
                    <button className={`md:hidden text-[#a7ebf2] p-2 z-20 ${isChatbotPage && user ? 'order-last' : ''}`} onClick={() => setIsOpen(!isOpen)} aria-label="Toggle navigation menu">
                    ...
                    <div className="hidden md:flex space-x-4 items-center">
                        ...
                        {/* --- MODIFIED: Desktop User Profile Indicator & Dropdown --- */}
                        {user && isChatbotPage && (
            ...
            <div className={`md:hidden absolute top-16 left-0 w-full bg-[#011c40] p-4 border-t border-gray-700 ${isOpen ? "block" : "hidden"}`}>
                <ul className="space-y-2">
                    {displayLinks.map((link) => (
                        <li key={link.href}>
                            <Link href={link.href} onClick={() => setIsOpen(false)} className={`w-full block text-left px-4 py-2 text-lg font-semibold rounded-md ${getMobileLinkStyles(link)}`}>
                                {link.label}
                            </Link>
                        </li>
                    ))}
                    {/* --- NEW: Logout button for mobile menu --- */}
                    {user && (
